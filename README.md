...
because I didn't have enough computer science credits).  In hindsight, that may
have been a poor decision, but I guess it passed the time between interviews.
...
you, person who may or may not be "man".
...
Everything is a block, which is a sequence of expressions.  Expressions in a
block must be terminated with a semicolon; inside a list they're terminated by
...
colon and a function block, and terminated by a semicolon as a normal expression
is (which is why functions in a block -- including the main program, which is
just a block itself -- are always followed by at least two semicolons, one for
the final expression in the function block in the definition, one for the
definition itself).
...
* `catch`: catches an exception and returns list `[<type> car[_] <stack>]`, if
  passed non-exception expression, returns `["ok" car[_]]`
